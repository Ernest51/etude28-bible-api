...
def cluster_for(book: str) -> str:
    return BOOK_CLUSTERS.get(book, "doctrine")

def pick(lst: List[str], rnd: random.Random, k: int = 2) -> List[str]:
    if not lst: return []
...
# -----------------------------
def generate_section(title: str, book: str, chapter: int, cl: str, rnd: random.Random) -> List[str]:
    hints = CLUSTER_HINTS.get(cl, [])
    chosen = pick(hints, rnd, k=min(2, max(1, len(hints)//2 or 1)))

    base = [
        f"- Contexte ({book} {chapter}) : {chosen[0] if chosen else 'thème du chapitre'}.",
        f"- Lien biblique : {chosen[1] if len(chosen) > 1 else 'cohérence du récit'}.",
    ...
    if "prière" in title.lower() and "ouverture" in title.lower():
        base = [
    ...
    if "plan d'action" in title.lower():
        base = [
